...
## 6. Endpoint Documentation Context (CRITICAL FOR ROUTING)
Before writing or updating tests for a specific feature, you MUST read its corresponding endpoint structure documentation to know the exact URLs, HTTP methods, and expected request/response structures. 
These files are located in the `docs/` folder, organized by feature. 

**Structure Example:**
- `docs/admin-desk/modules/modules-endpoint-structure.md`
- `docs/admin-desk/students/students-endpoint-structure.md`
- `docs/admin-desk/contracts/contracts-endpoint-structure.md`
- `docs/admin-desk/payments/payments-endpoint-structure.md`

Always refer to these documents to construct your `request(app).get('/exact/route')` calls accurately. DO NOT guess the routes.