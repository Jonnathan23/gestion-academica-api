...
*   **Interfaces (Contracts):** Definen los contratos obligatorios (`Repositories`, `DataSources`). Dictan *qué* debe poder hacerse, pero no *cómo*.
...
La consistencia de los registros contables, curriculares y de facturación nunca debe corromperse. En los *Datasources* (p.ej.: `contract.datasource.impl.ts`), utilizamos explícitamente `sequelize.transaction()` para controlar bloques de operaciones múltiples (como un `bulkCreate` y un ciclo iterativo de actualización).
